...
- Rent-to-revenue ratio benchmark: F&B healthy range is 6-10% of gross revenue. Calculate what annual revenue is needed for this rent to hit 8% (the midpoint benchmark): monthlyRent x 12 / 0.08
...
const CATEGORY_COLORS = {NEIGHBORHOOD_BAR:"#8B1A1A",DESTINATION_RESTAURANT:"#8B1A1A",FAST_CASUAL:"#8B1A1A",CAFE_COFFEE:"#8B1A1A",RETAIL_SPECIALTY:"#8B1A1A",NIGHTLIFE:"#8B1A1A"};
...
function ScoreArc({ score }) {
  ...
  const color = score >= 75 ? "#7ecb9c" : score >= 55 ? "#e6c96a" : "#e07c7c";
  return (
    ...
    <div style={{padding:"20px 0",borderBottom:"1px solid #eee"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:"8px"}}>
        ...
        <div style={{display:"flex",alignItems:"baseline",gap:"8px"}}>
          ...
          <span style={{fontFamily:"'Helvetica Neue',monospace",fontSize:"9px",color:"#bbb"}}>×{dim.weight}</span>
        </div>
      </div>
      ...
      <p style={{fontFamily:"'EB Garamond','Garamond',serif",fontSize:"16px",color:"#555",lineHeight:"1.7",margin:"0 0 10px"}}>{dim.summary}</p>
...
function Log({ lines }) {
  ...
  return (
    <div ref={ref} style={{fontFamily:"'Courier New',monospace",fontSize:"11px",color:"#2e7d52",background:"#f5f9f6",border:"1px solid #d4e8db",padding:"20px",borderRadius:"2px",height:"160px",overflowY:"auto",lineHeight:"2",textAlign:"left"}}>
      {lines.map((l,i) => <div key={i}><span style={{color:"#bbb"}}>›› </span>{l}</div>)}
...
export default function App() {
  ...
  const analyze = async () => {
    setLoading(true); setResult(null); setError(null); setLogs([]); setTab("overview");
    addLog("Initializing location intelligence scan...");
    addLog(`Target: ${address}`);
    addLog(`Concept: ${concept}`);
    if (monthlyRent) addLog(`Monthly rent provided: $${monthlyRent} — will calculate rent economics`);
    addLog("Classifying concept → selecting weight table...");
    const rentNote = monthlyRent ? `\nMonthly Rent: $${monthlyRent} — use this to calculate rent economics (implied annual, PSF estimate, revenue needed at 8% rent-to-sales ratio, verdict on whether rent is favorable/fair/high for this concept and market).` : "\nMonthly Rent: not provided — estimate market PSF only.";
    ...
    try {
      ...
      addLog("Calculating weighted composite score...");
      addLog("Building report...");
  ...
  const tabS = (t) => ({
    fontFamily: "'Courier New',monospace", fontSize: "10px", letterSpacing: "0.12em", textTransform: "uppercase",
    padding: "8px 18px", border: "none", borderBottom: tab === t ? "2px solid #c8a97e" : "2px solid transparent",
    background: "transparent", color: tab === t ? "#c8a97e" : "#555", cursor: "pointer"
  });

  const BH_RED = "#7a1515";
  const BH_RED_DARK = "#5c0f0f";
  ...
  return (
    <div style={{ background: "#fff", minHeight: "100vh", color: "#1a1a1a" }}>
      ...
      {/* Nav — matches Back-House burgundy */}
      <nav style={{ background: BH_RED, padding: "14px 48px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <a href="https://www.back-house.us" style={{ textDecoration:"none", display:"flex", alignItems:"center" }}>
          <img src="/logo.png" alt="Back-House" style={{ height: "28px", filter: "brightness(0) invert(1)" }} />
        </a>
        <span style={{ fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontSize: "10px", letterSpacing: "0.18em", color: "rgba(255,255,255,0.45)", textTransform: "uppercase" }}>Site Intel · Location Scoring</span>
      </nav>

      {/* Hero */}
      <div style={{ maxWidth: "780px", margin: "0 auto", padding: "72px 32px 56px", textAlign: "center" }}>
        <p style={{ fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontSize: "10px", letterSpacing: "0.25em", color: BH_RED, textTransform: "uppercase", marginBottom: "20px", fontWeight: "500" }}>Location Intelligence Engine</p>
        ...
        <div style={{ background: "#f8f7f5", border: "1px solid #e8e4de", padding: "40px", textAlign: "left" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "20px" }}>
            <div>
              ...
              <input value={address} onChange={e => setAddress(e.target.value)} style={inputStyle} placeholder="123 Court St, Brooklyn, NY" />
            </div>
            <div>
          ...
          <div style={{ marginBottom: "24px" }}>
            <label style={labelStyle}>Monthly Rent <span style={{ color: "#bbb", fontWeight: "400", letterSpacing: "0.05em", textTransform: "none" }}>— optional, unlocks rent economics analysis</span></label>
          ...
          <button onClick={analyze} disabled={loading}
            style={{ width: "100%", padding: "16px", background: loading ? "#ccc" : BH_RED, color: "#fff", border: "none", fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontSize: "11px", fontWeight: "600", letterSpacing: "0.22em", textTransform: "uppercase", cursor: loading ? "not-allowed" : "pointer", borderRadius: "2px" }}>
            {loading ? "Analyzing Location..." : "Score This Location →"}
          </button>
        </div>

        {loading && <div style={{ marginTop: "24px" }}><Log lines={logs} /></div>}
        {error && <div style={{ marginTop: "20px", background: "#fff5f5", border: "1px solid #fcc", padding: "16px", color: "#c0392b", fontFamily: "'Helvetica Neue',monospace", fontSize: "12px", textAlign: "left", borderRadius:"2px" }}>Error: {error}</div>}
      </div>
      ...
      {result && (
        <div ref={resultRef} style={{ animation: "fadeUp 0.6s ease", borderTop: `4px solid ${BH_RED}` }}>
            ...
            <div style={{ display: "flex", marginTop: "56px", borderTop: "1px solid #222", borderLeft: "1px solid #222" }}>
              {result.dimensions.map((d, i) => (
                <div key={i} style={{ flex: 1, padding: "20px 8px", borderRight: "1px solid #222", borderBottom: "1px solid #222", textAlign: "center" }}>
                  ...
                  <div style={{ fontFamily: "'Helvetica Neue',monospace", fontSize: "8px", color: "rgba(255,255,255,0.2)", marginTop: "5px" }}>wt {d.weight}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Tab bar */}
          <div style={{ borderBottom: "2px solid #eee", padding: "0 48px", display: "flex", gap: "0", background: "#fff", overflowX: "auto" }}>
            ...
            ).map(t => <button key={t} onClick={() => setTab(t.toLowerCase())} style={{
              fontFamily: "'Helvetica Neue',Helvetica,Arial,sans-serif", fontSize: "10px", letterSpacing: "0.15em",
              textTransform: "uppercase", padding: "16px 20px", border: "none", borderBottom: tab === t.toLowerCase() ? `3px solid ${BH_RED}` : "3px solid transparent",
              background: "transparent", color: tab === t.toLowerCase() ? BH_RED : "#999", cursor: "pointer", fontWeight: "600", whiteSpace: "nowrap", marginBottom: "-2px"
            }}>{t}</button>)}
          </div>
          ...
          <div style={{ maxWidth: "920px", margin: "0 auto", padding: "52px 32px" }}>
            ...
            {tab === "history" && (
              <div>
                ...
                {result.locationHistory?.length > 0
                  ...
                  : <p style={{ fontFamily: "'EB Garamond',serif", fontSize: "16px", color: "#aaa", fontStyle: "italic" }}>No prior tenant data found for this address.</p>
                }
            ...
            {tab === "transit" && result.transitSnapshot && (
              <div>
                ...
                <p style={{ fontFamily: "'EB Garamond',serif", fontSize: "16px", color: "#888", marginBottom: "32px", fontStyle: "italic" }}>Subway proximity weighted at {result.dimensions.find(d=>d.name==="Transit & Accessibility")?.weight * 100}% for this concept type.</p>
